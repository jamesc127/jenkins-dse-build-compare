...
//  val config      = ConfigFactory.parseFileAnySyntax(new File("./src/main/resources/stdevcheck.json"))
  ...
  val dse         = new dseSession(new InetSocketAddress(config.getString("dse.host"),config.getInt("dse.port")),!usejdbc)
  ...
  for (i <- queries.indices) {
    ...
    val json = parse(
      ...
      s"${config.getString("grafana.host")}" + ":" +
      s"${config.getInt("grafana.port").toString}" +
      s"${config.getString("grafana.path")}" +
      s"?query=${queries(i).getQuery}" +
      s"&start=${config.getLong("grafana.start-time").toString}" +
      s"&end=${config.getLong("grafana.end-time").toString}" +
    ...
    val insertMeanObject = dseInsert(build, longToTimestamp(config.getLong("grafana.end-time")),
      queries(i).getTestCase, "mean", meanAndStdDev._1.doubleValue())
    val insertStdevObject = dseInsert(build, longToTimestamp(config.getLong("grafana.end-time")),
    ...
    try {
      baselineBuildMean = if (usejdbc) jdbc.selectBaselineBuild(config.getInt("jenkins.baseline"), queries(i).getTestCase, "mean").getDouble(1)
      else dse.selectBaselineBuild(config.getInt("jenkins.baseline"), queries(i).getTestCase,"mean").one().getDouble("value")
    } catch { case e: Exception => println("Error retrieving baseline metric "+e.toString) }

    try {
      baselineBuildStdev = if (usejdbc) jdbc.selectBaselineBuild(config.getInt("jenkins.baseline"), queries(i).getTestCase, "stdev").getDouble(1)
      else dse.selectBaselineBuild(config.getInt("jenkins.baseline"), queries(i).getTestCase,"stdev").one().getDouble("value")
    } catch {
    ...
    val stdevCheck = bMath.stdevCheck(
      ...
      config.getDouble("jenkins.stdevtolerance"), queries(i).getAboveStdev)
